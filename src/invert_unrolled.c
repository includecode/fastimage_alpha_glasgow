...
void process_unrolled(u8 *frame)
{
  u64 end1 = 691200; //one Frame size / 4
  u64 end2 = 1382400;
  u64 end3 = 2073600;
  u64 end4 = 2764800;
  for (unsigned x = 0; x < end1; x += 32)
  {
    //Invert each colour component of every pixel
    frame[x] = ~frame[x];           //Red
  ...
  for (unsigned x = end1; x < end2; x += 32)
  {
    //Invert each colour component of every pixel
    frame[x] = ~frame[x];           //Red
    frame[x + 1] = ~frame[x + 1];   //Green
  ...
  for (unsigned x = end2; x < end3; x += 32)
  {
    //Invert each colour component of every pixel
    frame[x] = ~frame[x];           //Red
    frame[x + 1] = ~frame[x + 1];   //Green
  ...
  for (unsigned x = end3; x < end4; x += 32)
  {
    //Invert each colour component of every pixel
    frame[x] = ~frame[x];           //Red
    frame[x + 1] = ~frame[x + 1];   //Green
